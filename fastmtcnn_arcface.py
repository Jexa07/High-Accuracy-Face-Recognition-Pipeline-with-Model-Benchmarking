...
while True:
    ...
    # Detect faces using DeepFace's fastmtcnn
    face_objs = DeepFace.extract_faces(
        rgb_frame, detector_backend="fastmtcnn", enforce_detection=False
    )
    ...
    for face_obj in face_objs:
        ...
        try:
            ...
            if recognition_results and len(recognition_results[0]) > 0:
                ...
                identity = os.path.basename(result["identity"])  # Extract filename only
            ...
            known_faces = [folder.lower() for folder in os.listdir(db_path)]
            if identity.lower() in known_faces:
                identity = "Arpita"
    ...
    cv2.putText(frame, "DeepFace fastmtcnn + ArcFace", (10, 30),
                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
